...
def verification_loop_txt(model, task1_out, corpus, question, batch_size, top_k, evidence_type, class_type, model_mode,
                          vl_model, run_dir, args, wte_model, tokenizer, stop_tokens_tensor,
                          sub_tokens_tensor, rank_lm, logger, device):
    ...
    for q_key in tqdm.tqdm(task1_out):
        ...
        if args.use_prompt:
            ...
            masked_sentences.append(prompt)

        for text in E_pool.values():
            masked_sentences.append(text)

        #### reasoning_support_text
        gen_text = Get_shuffle_score(None, masked_sentences, model, vl_model, wte_model, tokenizer,
                                     None, stop_tokens_tensor, sub_tokens_tensor,
        ...
        reasoning_support_text = gen_text[0].lower().capitalize()
        ...
        for batch in corpus_loader:
            ...
            prompt = get_prompt_text(question, task1_out[q_key]['query'], reasoning_support_text, batch_corpus)
...
#     return final_out
#
# def verification_loop_img(model,task1_out,question,batch_size,top_k,evidence_type,
#                           model_mode,vl_model, run_dir,args,wte_model,tokenizer,stop_tokens_tensor,sub_tokens_tensor,rank_lm, logger, device):
#     start_time = time.time()
...
def verification_loop_multimodal(model, task1_out, txt_corpus, question, batch_size, top_k, evidence_type, class_type,
                                 model_mode, vl_model, run_dir, args, wte_model, tokenizer, stop_tokens_tensor,
                                 sub_tokens_tensor, rank_lm, logger, device):
    start_time = time.time()
    Transforms = T.Resize((224, 224))
    for q_key in tqdm.tqdm(task1_out):
        ...
        batch_image_embeds = []
        for img_path in E_pool_img.values():
            image = Image.open(img_path).convert("RGB")  # 打开图片
            image_tensor = T.PILToTensor()(image).to(device)  # 转为张量并移动到 GPU
            image_embedding = vl_model.get_image_embedding(image_tensor)  # 使用 CLIP 计算嵌入
            batch_image_embeds.append(image_embedding.unsqueeze(0))  # 添加 batch 维度
        ...
        for batch in corpus_loader:
            ...
            for txt_c in E_pool_txt:
                ...
                prompt = get_prompt_multimodal(task1_out[q_key]['query'], question, reasoning_support_text, txt_evidenc)
                if class_type == 'binary':
                ...
                for generated_text, batch_key, generated_text_proba in zip(generated_texts, batch_keys,
                    ...
                    task1_out[q_key]['top_verif_pred'].append(
                        {"corpus_key": batch_key, "generated-text": generated_text, "score": generated_text_proba,
                         "txt_corpus_key": txt_c, "flag": 0})

        combined_prompt = get_prompt_multimodal_multi(task1_out[q_key]['query'], question, reasoning_support_text,
                                                      list(E_pool_txt.values()))
        if class_type == 'binary':
            generated_texts, generated_texts_probas = model.get_response_YN(model_mode, images=batch_corpus,
                                                                            queries=[combined_prompt] * batch_size)
        else:
            generated_texts, generated_texts_probas = model.get_response_YNN(images=batch_corpus,
                                                                             queries=[combined_prompt] * batch_size)
        ...
        for generated_text, batch_key, generated_text_proba in zip(generated_texts, batch_keys, generated_texts_probas):
            task1_out[q_key]['top_verif_pred'].append(
                {"corpus_key": batch_key, "generated-text": generated_text, "score": generated_text_proba,
                 "txt_corpus_key": list(E_pool_txt.values()), "flag": 0})
...
def majority_voting(task1_out, run_dir):
    for q_id in task1_out:
        ...
        predictions_df['weight'] = predictions_df['flag'].apply(lambda x: 0.5 if x == 1 else 0.1)

        grouped = predictions_df.groupby('generated-text').agg(
            count=('generated-text', 'count'),
            max_score=('score', 'max'),
            weighted_score=('weight', 'sum')
        )

        grouped['weighted_score'] = grouped['count'] * grouped['weighted_score'] + grouped['max_score']

        grouped = grouped.sort_values(by=['count', 'weighted_score'], ascending=False)
...
def test(args):
    ...
    _, args = setup_with_args(args, 'verification/output/ir_llms/factify',
                              '{}-{}-{}'.format(args.mode, args.model_name.replace("/", "-"),
    ...
    if args.media == 'txt':
        ...
        model = my_Mistral_verification(model=args.model_name, tokenizer=args.model_name,
                                        max_length=args.max_seq_length)  # if two_level_prompotin=True
        ...
        verificaiton_out = verification_loop_txt(model, task1_out, corpus, args.prompt, args.batch_size, args.top_k,
                                                 args.evidence_type, args.class_type, 'binary', vl_model, args.run_dir,
                                                 args, wte_model, tokenizer, stop_tokens_tensor, sub_tokens_tensor,
                                                 None, None, device)
        ...
        # else:
        # tokenizer, stop_tokens_tensor,
        # sub_tokens_tensor, rank_lm, logger, device):
        verificaiton_out = verification_loop_multimodal(model, task1_out, text_corpus, args.prompt, args.batch_size,
                                                        args.top_k,
                                                        args.evidence_type, args.class_type, 'binary', vl_model,
                                                        args.run_dir,
                                                        args, wte_model, tokenizer, stop_tokens_tensor,
                                                        sub_tokens_tensor, None, None, device)
        final_out = majority_voting(verificaiton_out, args.run_dir)
...
def get_args():
    ...
    parser.add_argument("--prompt", type=str, help="LLM Prompt",
                        default='Does this image and text evidence pair support or refute this claim?answer with yes if it supports, answer with no if it refutes and answer with none if it does not provide enough information.')  # multiple class multimodal prompt with
    ...
    parser.add_argument('--conzic_sample', type=bool, default=True,
                        help='conzic sample means a way to process logits by conzic method'
    ...
    parser.add_argument("--prompt", type=list, default=[
        'Based on the evidence, integrate the information into a clear and logical objective reasoning support text to aid further reasoning.'])
    parser.add_argument('--vl_model', type=str, default=r'./clip-vit-base-patch32')
    parser.add_argument("--wte_model_path", type=str, default=r'G:/HuggingFace/all-Mini-L6-v2')
    parser.add_argument("--lm_model_path", type=str, default=r'./pretrain_model/CBART')
    parser.add_argument('--max_len', type=int, default=50, help='The maximum length of the generated sentence.')
    parser.add_argument('--min_len', type=int, default=20, help='The minimum length of the generated sentence.')
    ...
    parser.add_argument("--model_name",
                        default='xtuner/llava-llama-3-8b')  # {Open-Orca/Mistral-7B-OpenOrca, Salesforce/instructblip-flan-t5-xl , llava-hf/llava-1.5-7b-hf}
    ...
    parser.add_argument("--level1_prompt",
                        default="Is this image and text evidence sufficient to confirm or reject this claim?answer with yes if they are suffiecient and aswer with no if they are not enough information")
    ...
    parser.add_argument('--task1_out', help='input',
                        default='./retrieval/output/ir_llms/factify/00019-valid-Open-Orca-Mistral-7B-OpenOrca-2024-04-23_12-49-05')  # Factify validation set text evidence folder
    parser.add_argument('--task1_out_img', help='input',
                        default='./retrieval/output/ir_llms/factify/00022-valid-Salesforce-instructblip-flan-t5-xl-2024-05-06_20-01-18')  # VLM img evidence for Factify valid
